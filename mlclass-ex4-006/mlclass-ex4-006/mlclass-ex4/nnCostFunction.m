...
% add regularization terms to Theta1_grad and Theta1_grad
for bp_i = 1:size(Theta1,1)
	for bp_j = 2:size(Theta1,2) % because we dont regularize 1st column
		Theta1_grad(bp_i,bp_j) += lambda/m * Theta1(bp_i,bp_j);
	end;
end;

for bp_i = 1:size(Theta2,1)
	for bp_j = 2:size(Theta2,2) % because we dont regularize 1st column
		Theta2_grad(bp_i,bp_j) += lambda/m * Theta2(bp_i,bp_j);
	end;
end;		
